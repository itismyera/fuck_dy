...
def save(dict, isFav = False):
    ...
    global downloadNum
    num = str(downloadNum)
    timeStamp = int(dict['create_time'])
    timeArray = time.localtime(timeStamp)
    otherStyleTime = time.strftime("%Y-%m-%d %H-%M-%S", timeArray)

    if dict['desc'] == '':
        name = dict['user_name'] + '的作品({})'.format(otherStyleTime) + '.mp4'
    else:
        name = dict['desc'] + '({})'.format(otherStyleTime) + '.mp4'
...
def user_get_fav(sec_uid):
    '''
    传入目标用户的sec_uid对单个用户循环抓取所有收藏
    :param sec_uid:
    :return:
    '''
    index = 0
    ...
    while True:
        ...
        url = 'https://api3-normal-c-lq.amemv.com/aweme/v1/aweme/favorite/?invalid_item_count=0&is_hiding_invalid_item=0&max_cursor={}&sec_user_id={}&count=20&is_order_flow=0&storage_type=1&ts={}&_rticket={}&mcc_mnc=46000&'.format(
            max_cursor, sec_uid, min_t, max_t)
        data, max_cursor = get_data(url)
        index = index + 1
        if data is not None and max_cursor is not None:
            print("max_cursor_" + str(index), max_cursor)
            production_list = data_parse(data)
            for production in production_list:
                save(production, True)
        else:
            print('当前用户已抓完！')
            break
